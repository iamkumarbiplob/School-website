<?php
	...
	if(isset($_POST["submit"])){
		...
		if ($t_id==null || $password==null) {
		
			echo "Invalid ID or Password";
			header("Location: login.php");
		}
		else {
			$sql="SELECT `id`, `t_id`, `password`, `name`, `designation`, `mobile_no`, `image_url`, `present_address`, `parmanent_address` FROM `teacher` WHERE t_id='$t_id' AND password='$password'";

			$result = mysqli_query($conn,$sql);

			while($row=mysqli_fetch_assoc($result)){
				if ($row['t_id']==$t_id && $row['password']==$password){
					$name=$row['name'];
					session_start();
					$_SESSION['t_id']=$t_id;
					echo '<script type="text/javascript">alert("Hello, '.$name.' \n Wellcome to Student Panel");window.location =  "login_process.php";</script>';
				}
				else{
					echo "Dont";
				...
				<h1>Teacher Login</h1>
					<form action="" method="POST">
						<div class="textbox">
							...
							<input type="text" name="t_id" placeholder="Enter ID">
						</div>
						<div class="textbox">
